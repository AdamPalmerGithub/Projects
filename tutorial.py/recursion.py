...
"""
# ---> Iteration version
def factiorial_iterative(factorial_number):
    result = 1
    #iterate from 1 to n and multiply each number with the result
    for number in range(1, factorial_number + 1):    #example [1,2,3,4,5]
        result = result * number    #result* = number  <- shortcut
    return result

# Test the factorial_iterative function
print(factiorial_iterative(5)) #output 120
...
def fibonacci_iterative(n):
    ...
    for _ in range(2, n + 1):       #start at 2 as 0 + 1 are excluded      _ = variable name
